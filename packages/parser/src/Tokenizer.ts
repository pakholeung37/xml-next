...
export default class Tokenizer {
   ...
   */
  private fastForwardTo(c: number | number[]): boolean {
    if (typeof c === 'number') {
      c = [c]
    }
    while (++this.index < this.buffer.length + this.offset) {
      if (c.includes(this.buffer.charCodeAt(this.index - this.offset))) {
        return true
      }
    }
  ...
  private stateInNamedEntity(c: number): void {
    ...
    if (this.trieCurrent & BinTrieFlags.HAS_VALUE) {
      ...
      if (!this.allowLegacyEntity() && c !== CharCodes.Semi) {
        ...
        this.trieIndex +=
          1 + Number((this.trieCurrent & BinTrieFlags.MULTI_BYTE) !== 0)
        this.entityExcess = 0
        this.sectionStart = this.index + 1
      }
    }
